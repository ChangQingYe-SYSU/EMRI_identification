...
def TransformData(x,wis=None):
    N = len(x)
    y_t = pyfftw.empty_aligned(N, dtype='float64')
    yf = pyfftw.empty_aligned(int(N/2+1), dtype='complex128')
    fft_object = pyfftw.FFTW(y_t, yf, flags=('FFTW_ESTIMATE',))
    y_t = np.copy(x)
    y_f = np.copy(fft_object(y_t*dt))
    return (y_f[1:])


def Computeinner(data, x, Sa, freq,df,fmin=None, fmax=None):
    #{{{
    #fr = freq[1:]
    imax = len(freq)-1
    if (fmax != None):
        imax = np.argwhere(freq >fmax)[0][0] +1
    imin = 1
    if (fmin != None):
        imin = np.argwhere(freq >fmin)[0][0] +1

    SNR2 = (4.0*df) * np.sum(np.real(data[imin:imax]*np.conjugate(x[imin:imax])/Sa[imin:imax]))
